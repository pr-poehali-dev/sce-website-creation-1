...
const CreateReport: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Произошла ошибка при создании отчета');
    } finally {
    ...
    <AdminLayout>
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-white rounded-sm border border-sce-lightgray p-6 mb-6">
          ...
          <div>
            <label htmlFor="status" className="block text-sm font-medium text-sce-secondary mb-1">
              Статус отчета
            </label>
            <select
              id="status"
              ...
              onChange={handleChange}
            >
              <option value={ReportStatus.DRAFT}>Черновик</option>
              <option value={ReportStatus.SUBMITTED}>Отправить на рассмотрение</option>
              {user?.role === 'ADMIN' && (
                <>
                  <option value={ReportStatus.APPROVED}>Одобрить</option>
                  <option value={ReportStatus.REJECTED}>Отклонить</option>
                </>
              )}
            </select>
          </div>
